...
> - **Phase 3 is implemented and tested** with the same pattern: Syft, X.509 and
>   config engines work today; `cbomkit-theia` activates by configuration.
>   Container *image* scanning is built but unverified — no Docker daemon in this
>   environment.
...
| R17 | Scan **container images** | Phase 3 (implemented; untested — no Docker daemon here) | [~] built, unverified |
...
> **Status: complete.** 187 Go tests · 174 Python tests. Golden CBOM at
> `tests/golden/deployment-fixtures.cbom.json`, generated by the real scanner.

> **The plan assumed `cbomkit-theia` would be installed. It is not, and the
> Docker daemon is down**, so image pulls and theia's cert/key detection could
> not be run or measured here. Rather than leave the phase unverifiable, the
> same pluggable-engine pattern as Phase 2 was used:
>
...
- **Container image pulls** are implemented behind the same interface but untested: no Docker daemon in this environment. `image:` references resolve correctly and are never path-resolved (tested), and filesystem-only engines report an explicit gap rather than silently yielding nothing.
...
| **M3** | It finds crypto everywhere | 3, 4 | Certs, keys, libraries, declared TLS | **Phase 3 done**; Phase 4 pending |
